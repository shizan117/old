            ...
            <div class="col-12">
                <div class="card-box">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="p-20">
                                <form class="form-horizontal" role="form" action="{{ route('purchase.edit.post', $prs->id) }}" method="POST">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="input-group col-md-3">
                                            <input type="text" autocomplete="off" name="date" class="form-control"
                                                   ...
                                                   id="datepicker" value="{{ $prs->purchase_date }}">
                                    ...
                                    <table id="invoice-item-table" class="table table-bordered table-responsive-sm table-responsive-lg table-sm">
                                        ...
                                        <tr id="row_id_{{$i}}">
                                            <td class="text-center"><span id="sr_no">{{$i}}</span></td>
                                            <td>
                                                <select name="item_cat_name[]" id="item_cat_name{{$i}}" data-srno="{{$i}}" class="form-control pr_item_add">
                                                    <option value=''>Select Item Category</option>
                                                    @foreach ($cats as $cat)
                                                    <option value="{{ $cat->id }}" {{ ($pr_item->product->cat_id == $cat->id) ? 'selected':'' }}>{{ $cat->name }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
                                                <select name="item_name[]" id="item_name{{$i}}" data-srno="{{$i}}" class="form-control item_name_add">
                                                    <option value=''>Select Item Name</option>
                                                    <option value="{{ $pr_item->product_id }}" selected>{{ $pr_item->product->name }}</option>
                                                </select>
                                            </td>
                                            <td hidden>
                                                <input type="hidden" name="item_sl[]" id="item_sl{{$i}}" data-srno="{{$i}}" class="form-control item_sl" value="{{ $pr_item->product->serial_type }}"/>
                                            </td>
                                            @if($pr_item->product->serial_type == 2)
                                                @php($s_pr = \App\StockItem::where('product_id', $pr_item->product_id)->where('purchase_id', $prs->id)->select('serial')->first())
                                                @php($sku = $s_pr->serial)
                                                @php($readonly = '')
                                            @else
                                                @php($sku = '')
                                            ...
                                            @endif
                                            <td>
                                                <input type="text" name="item_sku[]" id="item_sku{{$i}}" data-srno="{{$i}}" {{ $readonly }} class="form-control" value="{{ $sku }}"/>
                                            </td>

                                            <td><input type="text" name="order_item_quantity[]" id="order_item_quantity{{$i}}" data-srno="{{$i}}" value="{{ $pr_item->qty }}" class="form-control order_item_quantity" /></td>
                                            <td><input type="text" name="order_item_price[]" id="order_item_price{{$i}}" data-srno="{{$i}}" value="{{ $pr_item->price }}" class="form-control number_only order_item_price" /></td>
                                            <td><input type="text" name="order_item_final_amount[]" id="order_item_final_amount{{$i}}" data-srno="{{$i}}" readonly value="{{ $pr_item->total_price }}" class="form-control order_item_final_amount" /></td>
                                            <td>
                                                @if($i > 1)
                                                    <button type="button" name="remove_row" id="{{$i}}" class="btn btn-danger btn-xs remove_row">X</button>
                                                @endif
                                            </td>
                                        </tr>
                                        @endforeach
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <label class="col-md-2 control-label"><b><span id="final_total_amt">{{ $prs->price }}</span></b></label>
                                        <input type="hidden" id="final_total_amt_val" class="form-control" name="final_total" value="{{ $prs->price }}">
                                    </div>
                                        ...
                                        <div class="col-md-4">
                                            <select name="account" id="account" class="form-control">
                                                ...
                                                @foreach ($accounts as $ac)
                                                    <option value='{{ $ac->id }}' {{ ($prs->transaction->account_id == $ac->id) ? 'selected':'' }}>{{ $ac->account_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group mb-0 justify-content-end row">
                                        <div class="col-10">
                                            <button type="submit" id="purchase-add" class="btn btn-info waves-effect waves-light">Edit Purchase</button>

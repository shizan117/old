    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card-box table-responsive">
                ...
                <div class="row">
                    <div class="col-md-6 d-md-flex align-items-start justify-content-between">
                        <h4 class="m-t-0 header-title py-2 py-md-0" style="color: #ffc107;">New & Old Client - Last 30 Days</h4>
                        <div class="w-25 pb-3 d-md-block d-none">
                            <select name="clientType" id="clientType" class="form-control bg-secondary">
                                {{-- <option value="">All Clients</option> --}}
                                <option value="newClients" {{ request('clientType') == 'newClients' ? 'selected' : '' }}>New
                                    Clients</option>
                                <option value="oldClients" {{ request('clientType') == 'oldClients' ? 'selected' : '' }}>Old
                                    Clients</option>
                            </select>
                        </div>

                        <div class="w-75 pb-3 d-md-none">
                            <select name="clientType" id="clientType" class="form-control bg-secondary">
                                {{-- <option value="">All Clients</option> --}}
                                <option value="newClients" {{ request('clientType') == 'newClients' ? 'selected' : '' }}>New
                                    Clients</option>
                        ...
                        <tbody>
                            @foreach ($clientData as $dataClient)
                                <tr>
                                    ...
                                    {{-- <td>{{ $dataClient->reseller->resellerName }}</td> --}}
                                    {{-- <td>Reseller Name ..</td> --}}
                                    <td>{{ $status }}</td>
                                    ...
                                    <td class="hidden-print">
                                        <span style="display: flex; justify-content: center;">
                                            ...
                                            @if ($user->hasAnyRole('Super-Admin', 'Reseller') || $user->can('client_edit'))
                                                {{-- <a href="{{ route('client.edit', $dataClient->id) }}" class="btn-edit"><i class="fa fa-edit"></i></a> --}}
                                            @endif
                                            {{-- <a href="#" class="btn-del"><i class="fa fa-trash-o"></i></a> --}}
                                            {{-- @if ($dataClient->status == 'On' && setting('using_mikrotik') && ($user->hasAnyRole('Super-Admin', 'Reseller') || $user->can('client_bandwidth graph')))
                                        <a class="btn-show" id="chartShow" data-toggle="modal" style="cursor: pointer;" data-name="{{ $dataClient->client_name }}" data-target="#trafficView" data-id="{{ $dataClient->id }}">
                                    ...
                                    <a href="{{ route('client.pay.due', $dataClient->id) }}" class="btn-show"><i class="fa fa-product-hunt text-primary"></i></a>
                                    @endif
                                    @endif --}}
                                            {{-- @if (request()->is('adminisp/clients/old-clients') && ($user->hasAnyRole('Super-Admin', 'Reseller') || $user->can(['client_delete'])))
                                    ...
                                    @endif --}}
                                            ...
                                            ?>
                                            {{-- <a href="https://api.whatsapp.com/send/?phone={{ $to }}" title="Contact With Client"
                                    class="text-success waves-effect waves-light" style="font-size: 18px;" target="_blank">
        ...
        <div class="modal-dialog traffic-graph modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <label class="modal-title text-text-bold-600 text-warning" id="myModalLabel33" style="font-size: 20px;">
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#clientType").on('change', function() {
                ...
                window.location.href = "{{ route($route_url) }}" + "?clientType=" + clientType;
            })
        });
    </script>
@endsection

            ...
            <div class="card-box table-responsive">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="btn-group m-b-10">
                            <div class="btn-group m-b-10">
                                {{--@if($role_id != 4)--}}
                                    {{--<a href="{{ route('branch.expanse.list') }}" class="btn btn-secondary">Branches Expanse List</a>--}}
                                    {{--<a href="{{ route('expanse.list') }}" class="btn btn-success">All Expanse List</a>--}}
                                {{--@endif--}}
                                @if($user->hasAnyRole('Super-Admin','Reseller') || $user->can('expense_add'))
                                <a href="{{ route('expanse.add') }}" class="btn btn-primary">Add Expense</a>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <table id="datatable"
                    ...
                    @foreach ($expanses as $expanse)
                        @php($total_amount += $expanse->amount )
                        <tr>
                            ...
                            <td>{{ $expanse->note }}</td>
                            <td class="text-right">{{ number_format($expanse->amount,2) }}</td>
                            <td>{{ $expanse->user->name }}</td>
                            {{--@if($role_id != 4)--}}
                                {{--@php((empty($expanse->branchId)) ? $branch_name = '-' : $branch_name = $expanse->branch->branchName)--}}
                                {{--<td>{{ $branch_name }}</td>--}}
                            {{--@endif--}}
                            ...
                            <td class="hidden-print">
                                ...
                                @if($user->hasAnyRole('Super-Admin','Reseller') || $user->can('expense_edit'))
                                <a href="{{ route('expanse.edit', $expanse->id) }}" class="btn-edit">

        ...
        <div class="col-12">
            <div class="card-box">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="p-20">
                            <form class="form-horizontal" role="form" action="{{ route('product.edit.post', $product->id) }}"
                                  method="POST">
                                {{ csrf_field() }}
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-10 {{ $errors->has('single_unit_serial') ? 'has-error' : '' }}">
                                        <select class="form-control" name="single_unit_serial">
                                            ...
                                            <option value="1" {{ ($product->serial_type == 1) ? 'selected':'' }}>Yes</option>
                                            <option value="2" {{ ($product->serial_type == 2) ? 'selected':'' }}>No</option>
                                        </select>
                                    ...
                                    {{--<div class="col-10 {{ $errors->has('branch') ? 'has-error' : '' }}">--}}
                                        {{--<select class="form-control" name="branch">--}}
                                            ...
                                            {{--@foreach($branches as $branch)--}}
                                                {{--<option value="{{$branch['branchId']}}" {{ ($product->branchId == $branch->branchId) ? 'selected':'' }}>{{$branch['branchName']}}</option>--}}
                                            {{--@endforeach--}}
                                        {{--</select>--}}

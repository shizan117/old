...
<header>
    <div class="name">
        ...
        <img src="{{ asset("assets/images/".$setting['logo']) }}" class="img-thumbnail" width="100%"/>
        {{--<span>user</span>--}}
        {{--<span class="letter_a">A</span> <span>ctivity</span>--}}
    </div>
    <div>
        <span class="text_light" style="color: yellow;">
            Notice:
            {{ config('activitylog.delete_records_older_than_days') }}
            days old log data will be deleted automatically </span>

    </div>
    <div class="actions">
        <a class="btn" href="{{ route('admin.dashboard') }}">Admin Panel</a>
    </div>
</header>
<section class="content">
    <div class="top_content">
        ...
        <div class="top_content_right">
            <div class="filter_item">
                <label>USER</label>
                <select ng-model="filter.user_id">
            ...
            <div class="filter_item">
                ...
                <select ng-model="filter.log_event">
        ...
        <div class="responsive_table">
            <table>
                ...
                <tr ng-repeat="log in data |filter: selectedType track by $index">
                    ...
                    <td>@{{ log.log_name }}</td>
                    <td>@{{ log.properties.subject_info }}</td>
                    <td ng-switch="log.event">
                        <span ng-switch-when="deleted" class="badge emergency">@{{log.event}}</span>

                        <span ng-switch-when="created" class="badge info">@{{log.event}}</span>
                        <span ng-switch-when="created" class="lbl_table"></span>
                        <span ng-switch-when="updated" class="badge warning">@{{log.event}}</span>

                        <span ng-switch-when="updated|deleted"
                              ng-switch-when-separator="|" class="lbl_table"></span>

                        <span ng-switch-default class="badge debug">@{{log.event}}</span>
                    </td>

                    <td>
                        <strong>@{{ log.causer.name }}</strong><br>
                        <span class="text_light">@{{ log.causer.email }}</span>
                    </td>
    ...
    {{--<footer>--}}
        ...
        {{--<div class="footer_right">--}}
            {{--<span class="text_light"> {{ config('activitylog.delete_records_older_than_days') }} days old log data will be deleted automatically </span>--}}
            ...
            <div class="popup_content">
                <table style="width: 96%;">
                    ...
                    <tr>
                        ...
                        <td ng-switch="selected.event">
                            <span ng-switch-when="deleted"
                                  ng-switch-when-separator="|" class="badge emergency">@{{selected.event}}</span>

                            <span ng-switch-when="created" class="badge info">@{{selected.event}}</span>
                            <span ng-switch-when="updated" class="badge warning">@{{selected.event}}</span>
                            <span ng-switch-default class="badge debug">@{{selected.event}}</span>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="field_cell">IP Address</td>
                        <td>@{{ selected.properties.ip }}</td>
                    </tr>
                    <tr>
                        <td class="field_cell">Done by</td>
                        <td>@{{ selected.causer.name }} - <span class="text_light">@{{ selected.causer.email }}</span></td>
                    </tr>
                    <tr>
                        <td class="field_cell">Subject Info</td>
                        <td>#@{{ selected.subject_id }} - @{{ selected.properties.subject_info }}</td>
                    </tr>
                </table>
                ...
                <div class="responsive_table">
                    <table style="width: 96%;">
                        ...
                        <tr>
                            <td>FIELD</td>
                            <td>@{{ selected.event==='updated'?'CURRENT':'DATA' }}</td>
                            <td ng-show="selected.event==='updated'">PREVIOUS</td>
                        </tr>
                        </thead>
                        <tbody>

                        <tr ng-show="selected.event!=='deleted'" ng-repeat="(field,value) in selected.properties.attributes">
                            <td class="field_cell">@{{ field }}</td>
                            <td>@{{ value }}</td>
                            <td ng-show="selected.event==='updated'">
                                @{{ selected.properties.old[field] }}
                            </td>
                        </tr>

                        <tr ng-show="selected.event==='deleted'" ng-repeat="(field,value) in selected.properties.old">
                            <td class="field_cell">@{{ field }}</td>
                            <td>@{{ value }}</td>
                        </tr>
                ...
                <br>
                {{--<div class="responsive_table" ng-if="selected.event==='updated' && editHistory.length > 0">--}}
    ...
    @include('admin.pages.activity-log.script')
</section>

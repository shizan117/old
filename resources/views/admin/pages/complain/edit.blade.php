            ...
            <div class="col-12">
                <div class="card-box">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="p-20">
                                <form class="form-horizontal" role="form" action="{{ route('complain.update',$complain->id) }}" method="POST">
                                    {{ csrf_field() }}
                                    {{--<input type="hidden" name="{{$complain->id}}">--}}
                                    <div class="form-group row">
                                        <div class="col-4 {{ $errors->has('client_id') ? 'has-error' : '' }}">
                                            <label class="col-form-label">Client Name</label>
                                            <select class="form-control select2" name="client_id" id="client">
                                                <option value="">Select Client Name</option>
                                                @foreach($clients as $client)
                                                    <option value="{{$client['id']}}" {{ $complain->client_id==$client['id']?'selected':'' }}>{{$client['client_name'].'-'.$client['username']}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-4 {{ $errors->has('title') ? 'has-error' : '' }}">
                                        ...
                                        <div class="col-4 {{ $errors->has('assign_to') ? 'has-error' : '' }}">
                                            <label class="col-form-label">Assign To</label>
                                            <select class="form-control select2" name="assign_to" id="client">
                                                <option value="">Select User</option>
                                                @foreach($users as $user)
                                                    <option value="{{ $user->id }}" {{ $complain->assign_to==$user->id?'selected':'' }}>{{ $user->name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-12 {{ $errors->has('action_taken') ? 'has-error' : '' }}">
                                            ...
                                            <textarea class="form-control" name="action_taken">{{ $complain->action_taken }}</textarea>

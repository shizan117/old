                    ...
                    @foreach ($complains as $complain)
                        <tr>
                            ...
                            <td class="hidden-print">
                                @if($user->hasAnyRole('Super-Admin','Reseller') || $user->can('ticket_mark as solved'))
                                <a href="{{ route('complain.solve', $complain->id) }}"
                                   onclick="return confirm('Mark this complain as solved?')"
                                   class="btn-show" title="Mark as solved">
                                ...
                                @if($user->hasAnyRole('Super-Admin','Reseller') || $user->can('ticket_delete'))
                                <a href="" onclick="return myFunction('{{ $complain->client->client_name }}')" class="btn-del" title="Delete">
                                    <i class="fa fa-trash-o"></i>
                                </a>
                                <form id="delete" action="{{ route('complain.delete', $complain->id) }}" method="POST" style="display: none;">
                                    {{ csrf_field() }}
                                </form>
                                @endif
...
@section('custom_js')
    ...
    <script>
        function myFunction(name) {
          if(confirm("Are you sure to delete "+ name +"\'s complain?")){

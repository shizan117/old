            ...
            <div class="col-8">
                <div class="card-box">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="p-20">
                                <form class="form-horizontal" role="form" action="{{ route('account.edit.post', $accountData->id) }}" method="POST">

                                    {{ csrf_field() }}
                                    <div class="form-group row">
                                        <label class="col-3 col-form-label">Account Name</label>
                                        <div class="col-9 {{ $errors->has('account_name') ? 'has-error' : '' }}">
                                            <input type="text" class="form-control" name="account_name" value="{{ $accountData->account_name }}" placeholder="Enter Account Name">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-9 {{ $errors->has('account_type') ? 'has-error' : '' }}">
                                            ...
                                            <select class="form-control" name="account_type" required>
                                                <option value="Cash" {{ ($accountData->account_type == 'Cash') ? 'selected':'' }}>Cash Account</option>
                                                <option value="Bank" {{ ($accountData->account_type == 'Bank') ? 'selected':'' }}>Bank Account</option>
                                                <option value="bkash" {{ ($accountData->account_type == 'bkash') ? 'selected':'' }}>bKash</option>
                                                <option value="Rocket" {{ ($accountData->account_type == 'Rocket') ? 'selected':'' }}>Rocket</option>
                                                <option value="Nagad" {{ ($accountData->account_type == 'Nagad') ? 'selected':'' }}>Nagad</option>
                                            </select>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-9 {{ $errors->has('account_number') ? 'has-error' : '' }}">
                                            <input type="text" class="form-control" name="account_number" value="{{ $accountData->account_number }}" placeholder="Enter Account Number">
                                            ...
                                            {{--<div class="col-10 {{ $errors->has('branchId') ? 'has-error' : '' }}">--}}
                                                {{--<select class="form-control" name="branchId">--}}
                                                    ...
                                                    {{--@foreach($branchs as $branch)--}}
                                                        {{--<option value="{{$branch['branchId']}}" {{ ($accountData->branchId == $branch->branchId) ? 'selected':'' }}>{{$branch['branchName']}}</option>--}}
                                                    {{--@endforeach--}}
                                                {{--</select>--}}

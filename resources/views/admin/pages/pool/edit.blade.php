                        ...
                        <div class="col-12">
                            <div class="p-20">
                                <form class="form-horizontal" role="form"
                                      action="{{ route('pool.edit.post', $poolData->id ) }}" method="POST">

                                    {{ csrf_field() }}
                                    <div class="form-group row">
                                        <label class="col-2 col-form-label">Pool Name</label>
                                        <div class="col-10 {{ $errors->has('pool_name') ? 'has-error' : '' }}">
                                            <input type="text" class="form-control" name="pool_name"
                                                   value="{{ $poolData->pool_name }}" placeholder="Enter Pool Name">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-10 {{ $errors->has('range_ip') ? 'has-error' : '' }}">
                                            <input type="text" class="form-control" name="range_ip"
                                                   value="{{ $poolData->range_ip }}"
                                                   placeholder="Enter Pool IP (192.168.1.2-192.168.1.254)">
                                        ...
                                        <div class="col-10 {{ $errors->has('server_id') ? 'has-error' : '' }}">
                                            <select class="form-control" name="server_id">
                                                @foreach ($serverData as $server)
                                                    @if ($poolData->server_id == $server->id)
                                                        {{ $select = 'selected' }}
                                                    @else
                                                        {{ $select = '' }}
                                                    @endif
                                                    <option value="{{ $server->id }}" {{ $select }}>{{ $server->server_name }}</option>
                                                @endforeach
                                            </select>
